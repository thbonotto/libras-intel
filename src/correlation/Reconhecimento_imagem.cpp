...
vector<int> * Reconhecimento_imagem::reconhecer_imagem_vector(Mat realMap){
    ...
    for(i=0;i<26;i++){
        if(abs(correlation_result[i]) > 0.1){
            elements->push_back(i);
        }
    }
    return elements;
}
char Reconhecimento_imagem::reconhecer_imagem_int(Mat realMap){
    double value=0.0;
    char *c = new char;
    int i;
    pthread_t threads[26];
    ...
    for(i=0;i<26;i++)
        pthread_join(threads[i],NULL);
    for(i=0;i<26;i++){
        if(abs(correlation_result[i]) > value){
            value = abs(correlation_result[i]) ;
           c = const_cast<char*>(alfabeto[i].c_str());
        }
    }
    return *c;
}
...
char Reconhecimento_imagem::reconhecer_imagem_ext(Mat realMap){
    Mat slamMap;
    realMap.convertTo(realMap,CV_8UC1);
    Correlacao *im1 = new Correlacao();
    im1->mount_vector(realMap);
    ...
    for(i=0;i<26;i++){
        Correlacao *im2 = new Correlacao();
       // cout << "Letra"<< alfabeto[i].c_str() << endl;
        slamMap = imread("../../img/Tratada/Externo/"+alfabeto[i]+".jpg");
        slamMap.convertTo(slamMap,CV_8UC1);
        im2->mount_vector(slamMap);
        maxVal = im1->pearson_correlation(im2->_feature);
...
char Reconhecimento_imagem::reconhecer_imagem_withvector(Mat realMap,vector<int>* letters){
    ...
    for(i=0;i<letters->size();i++){
        if(abs(correlation_result[letters->at(i)]) > abs(corr)){
            corr = correlation_result[letters->at(i)];
            c = const_cast<char*>(alfabeto[letters->at(i)].c_str());
        }
    }
  cout << corr << endl;
