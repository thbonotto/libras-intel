...
int main (int argc, char** argv){	
	...
	realMapPath = argv[1];
	slamMapPath = argv[2];
	Mat realMap = imread(realMapPath, -1);
//	Mat realMap = (Tratamento_imagem::filtro_cinza(imread(realMapPath, -1)))>128; //Get the real map image. 0 is grayscale. -1 is original image.
	Mat slamMap = /*(Tratamento_imagem::filtro_cinza(*/imread(slamMapPath, -1)/*))>128*/; //Get the slam map image. 0 is grayscale. -1 is original 
	//Bad parameters handling.
