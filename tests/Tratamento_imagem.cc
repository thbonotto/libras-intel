 ...
 */
static Mat Tratamento_imagem::filtro_cinza(Mat image) {
	Mat gray_image;
 ...
 */
static Mat Tratamento_imagem::filtro_hsv(Mat image) {
	Mat hsv_image;
...
/*Função que faz um blur na imagem, da uma embaçada na imagem;
 ...
 return -> retorna a imagem trabalhada;
 */
static Mat Tratamento_imagem::blur_imagem(Mat image, int modo, int MAX_KERNEL_LENGTH) {
	Mat aux_image;
	switch (modo) {
	case 1:
 ...
 */
static void Tratamento_imagem::mostra_imagem(Mat image, String nome_janela) {
...
/*Função que aplica metodo canny para destacar contornos na imagem
 ...
 return -> retorna a imagem trabalhada;
 */
static Mat Tratamento_imagem::canny(Mat image, int thresh) {
 ...
 */
static Tratamento_imagem::Contorno Tratamento_imagem::find_contour(Mat image) {
 ...
 */
static Mat Tratamento_imagem::draw_contour(Tratamento_imagem::Contorno contornos){
...
/*Função que cria uma imagem a partir dos contornos
 ...
 return -> retorna imagem desenhada a partir do contorno
 */
static Mat Tratamento_imagem::draw_contour_image(Mat image){
 ...
 */
static Mat Tratamento_imagem::hue_image(Mat image) {
	Mat hue;
...
/*Função que aplica um alto contraste na foto
 param
 	 ...
 	 int bins -> grau de BackProject

 return -> retorna a imagem trabalhada;

 */
static Mat Tratamento_imagem::Hist_and_Backproj(Mat image,int bins) {
 ...
 */
static Mat Tratamento_imagem::threshold_image(Mat image, int mode,int intensity){
	Mat aux_image; 
 ...
 */
static Mat Tratamento_imagem::image_scale(Mat image,int mode,int proporcao)
{
...
static Scalar Tratamento_imagem::getMSSIM(const Mat& i1, const Mat& i2) {
...
static double Tratamento_imagem::getPSNR(const Mat& I1, const Mat& I2) {
	Mat s1;
...
static double Tratamento_imagem::avg_similarity(Mat img1,Mat img2){
	double PSNR=getPSNR(img1,img2);
	Scalar mssim=getMSSIM(img1,img2);
	return ((mssim[0]*PSNR+mssim[1]*PSNR+mssim[1]*PSNR)/3);
	}
